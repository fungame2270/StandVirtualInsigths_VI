...
function MeanPriceGraph({data, columns, city}) {
    ...
    useEffect(() => {
        ...
        const dados = data.filter((d) => city === "" ||d.City === city);
        console.log(dados)
        const groupedData = d3.group(dados, (d) => d[mode]);
        ...
        chartGroup
            ...
            .attr("fill", "steelblue")
            .on("mouseover",(e,d)=>{
                setToolTip({visible: true,x:e.clientX,y:e.clientY,content:`${d.Title} : ${Math.round(d.Price)}`})
            })
            .on("mousemove",(e,d)=>{
                setToolTip({visible: true,x:e.clientX,y:e.clientY,content:`${d.Title} : ${Math.round(d.Price)}`})
            })
    ...
    return(
        ...
        <div className="flex flex-col items-center">
            <div className="flex w-[600px]">
                <div className="ml-auto">Mean Price by {mode}</div>
        ...
        <AnimatePresence>
            ...
            <motion.div className="fixed bg-gray-200 p-2 rounded-md" ref={toolTipRef}
                style={{top:toolTip.y - 40,left:toolTip.x - toolTipSize.width/2, pointerEvents:"none"}}
